...
const getProcessingTimeInMillis = (time: [number, number]): string => {
...
export default function logger(req: Request, res: Response, next: NextFunction) {
    ...
    const startText = `START:${getProcessingTimeInMillis(start)}`;
    const idText = `[${id}]`;
    ...
    res.once('finish', () => {
      ...
      const endText = ` END:${getProcessingTimeInMillis(end)}`;
